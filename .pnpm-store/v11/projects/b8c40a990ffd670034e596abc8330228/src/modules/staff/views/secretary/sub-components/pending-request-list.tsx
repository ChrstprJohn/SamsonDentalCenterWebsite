...
export function PendingRequestList(props: PendingRequestListProps) {
  return (
    <div className="lg:col-span-5 border border-card-border bg-card rounded-3xl p-5 shadow-sm overflow-hidden flex flex-col">
      <div className="text-sm font-bold text-text-primary mb-3">Pending Requests ({props.appointments.length})</div>
      {props.isLoading ? (
          ...
          <table className="w-full text-left text-xs border-collapse">
            ...
            <tbody>
              {props.appointments.length === 0 ? (
                <tr><td colSpan={3} className="py-12 text-center text-text-muted">No pending requests at this time.</td></tr>
              ) : (
...
function PendingRequestRow({ appointment, isSelected, onSelect }: { appointment: any; isSelected: boolean; onSelect: (id: string) => void }) {
