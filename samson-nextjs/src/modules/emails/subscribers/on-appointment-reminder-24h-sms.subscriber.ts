...
export const onAppointmentReminder24hSmsSubscriber = {
  /**
   * Handles APPOINTMENT_REMINDER_24H_SMS.
   * Sends SMS reminder 24 hours before appointment.
   */
  async handle(payload: Record<string, any>): Promise<void> {
    ...
    if (appError || !appointment) {
      console.error(`[24H SMS Reminder] Failed to fetch appointment: ${appError?.message || 'Not found'}`);
      return;
    }

    const channel = (appointment as any).confirmation_channel || 'EMAIL';
    if (channel === 'NONE' || channel === 'EMAIL') {
      console.info(`[24H SMS Reminder] Skipping SMS dispatch for appointment ${appointmentId}: Confirmation channel is ${channel}.`);
      return;
    }
    ...
    if (!phoneNumber) {
      console.warn(`[24H SMS Reminder] Skipping SMS dispatch for appointment ${appointmentId}: No phone number found.`);
      return;
    }

    const dateStr = formatShortDate(appointment.date);
    const timeStr = formatClinicTime(appointment.start_time);
    const message = `Samson Dental: Reminder - You have an appt tomorrow ${dateStr} at ${timeStr}. Reply or call 0917-123-4567 to modify.`;

    await SmsService.sendSms(phoneNumber, message);

    await supabaseAdmin
      .from('appointments')
      .update({ reminder_24h_sent: true, sms_reminder_24h_sent: true })
      .eq('id', appointmentId);
  },
};
