-- Migration: Migrate Time Preference to Preferred Start Time
-- Date: 2026-07-12

-- 1. Alter appointment_inquiries table
ALTER TABLE public.appointment_inquiries DROP CONSTRAINT IF EXISTS check_time_preference;
ALTER TABLE public.appointment_inquiries RENAME COLUMN time_preference TO preferred_start_time;

-- 2. Alter appointments table
ALTER TABLE public.appointments DROP CONSTRAINT IF EXISTS check_appointments_time_preference;
ALTER TABLE public.appointments DROP CONSTRAINT IF EXISTS check_appointments_proposed_time_preference;

ALTER TABLE public.appointments RENAME COLUMN time_preference TO preferred_start_time;
ALTER TABLE public.appointments RENAME COLUMN proposed_time_preference TO proposed_preferred_start_time;

-- 3. Recreate submit_booking_transaction to accept preferred_start_time
DROP FUNCTION IF EXISTS public.submit_booking_transaction(
    UUID, UUID, UUID, DATE, TIMESTAMPTZ, TIMESTAMPTZ, TEXT, UUID, TEXT, TEXT, DATE, dependent_relationship, TEXT, TEXT, doctor_assignment_source, TEXT
);
...
-- 4. Recreate request_reschedule_transaction to accept proposed_preferred_start_time
DROP FUNCTION IF EXISTS public.request_reschedule_transaction(
    UUID, UUID, TEXT, TEXT, DATE, TIMESTAMPTZ, TIMESTAMPTZ, UUID, TEXT
);
...
CREATE OR REPLACE FUNCTION public.request_reschedule_transaction(
    ...
    p_proposed_preferred_start_time TEXT DEFAULT NULL
...
DECLARE
    ...
    v_has_reschedule_pref_col BOOLEAN;
BEGIN
    ...
    -- Check if column proposed_preferred_start_time exists, fallback to proposed_time_preference if this function runs pre-migration
    SELECT EXISTS (
        SELECT 1 
        FROM information_schema.columns 
        WHERE table_name='appointments' AND column_name='proposed_preferred_start_time'
    ) INTO v_has_reschedule_pref_col;

    -- 2. Update appointment status and store proposed schedule atomically
    IF v_has_reschedule_pref_col THEN
        ...
        SET
            ...
            proposed_preferred_start_time    = p_proposed_preferred_start_time,
            updated_at                       = NOW()
        WHERE id = p_appointment_id
        RETURNING * INTO v_updated_row;
    ELSE
        -- Fallback for backwards compatibility if run mid-process
        UPDATE public.appointments
        SET
            status                           = 'RESCHEDULE_REQUESTED'::public.appointment_status,
            status_reason                    = p_reason,
            ...
            proposed_end_time                = p_proposed_end_time,
            proposed_doctor_id               = p_proposed_doctor_id,
            proposed_time_preference         = p_proposed_preferred_start_time,
            updated_at                       = NOW()
        WHERE id = p_appointment_id
        RETURNING * INTO v_updated_row;
    END IF;
