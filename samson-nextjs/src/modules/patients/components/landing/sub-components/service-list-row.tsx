...
export function ServiceListRow({ nr, title, onClick }: ServiceListRowProps) {
  return (
    ...
    >
      ...
      <span className="relative z-10 text-base sm:text-lg md:text-base lg:text-2xl font-josefin font-normal text-white/75 group-hover:text-white transition-colors w-12 sm:w-20 md:w-16 lg:w-24 text-left">
        {nr}
      </span>
      <h4 className="relative z-10 font-josefin text-lg sm:text-xl md:text-xl lg:text-3xl font-normal tracking-tight text-white/90 group-hover:text-white transition-colors flex-1 text-center leading-[1.1]">
        <ServiceTitle title={title} />
      </h4>
      <div className="relative z-10 w-12 sm:w-20 md:w-16 lg:w-24 flex justify-end">
        <div className="w-10 h-10 sm:w-12 sm:h-12 md:w-10 md:h-10 lg:w-14 lg:h-14 bg-white/5 group-hover:bg-[#D94E4E] rounded-full border border-white/10 flex items-center justify-center text-white/80 group-hover:text-white transition-all duration-305">
