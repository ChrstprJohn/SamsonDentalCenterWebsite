-- Migration: allow fetching a single chat thread by appointment id (deep link support).
-- /secretary-v2/chat?id=<appointmentId> notifications must resolve the exact thread
-- regardless of tab/search/unread filters, so the secretary view can pin it to the list.
...
REVOKE ALL ON FUNCTION public.get_secretary_chat_threads_page_staff(INT, TIMESTAMPTZ, UUID, TEXT, TEXT, BOOLEAN, UUID) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.get_secretary_chat_threads_page_staff(INT, TIMESTAMPTZ, UUID, TEXT, TEXT, BOOLEAN, UUID) TO authenticated, service_role;

-- CREATE OR REPLACE with a new parameter count leaves the old overloads behind,
-- making named-arg calls ambiguous. Drop the 6-arg versions (wrapper first, the
-- inner one depends on nothing but the wrapper references it).
