...
- [x] **Phase 1: Standardize Hook Names & Add Sibling Tests**
  - [x] Rename all `*.hook.ts` and `*.hook.spec.ts` custom hook files to `*.ts` and `*.spec.ts` respectively.
  - [x] Create Vitest sibling test files for all the 9 missing hooks and the 1 missing server action listed above.
  - [x] Update all imports/mocks to the standardized hook filenames.

- [x] **Phase 2: Extract Reusable Scheduling Logic (Fix DRY Violations)**
  - [x] Create a single custom hook `use-booking-scheduler.ts` under `src/modules/appointments/hooks/shared/` to coordinate calendar dates queries, available doctors, and slot resolutions.
  - [x] Use Zod schema validation for scheduling inputs.
  - [x] Add sibling coverage for scheduler validation and action delegation.

- [x] **Phase 3: Refactor Landing Pages & Forms**
  - [x] Refactor [use-landing-view.ts](file:///c:/Users/picar/Desktop/samson-website/samson-nextjs/src/modules/patients/hooks/landing/use-landing-view.ts) to utilize Zod + React Hook Form for contact inquiries.
  - [x] Split [contact-section.tsx](file:///c:/Users/picar/Desktop/samson-website/samson-nextjs/src/modules/patients/components/landing/contact-section.tsx) and extracted contact info, form, fields, calendar, and contact-section state into focused sibling files.
  - [x] Split landing page sections that exceeded 150 lines (`about-section.tsx`, `gallery-section.tsx`, `journey-section.tsx`, `services-section.tsx`, `testimonials-section.tsx`).
  - [x] Added sibling hook coverage for new landing controllers (`use-contact-section.ts`, `use-gallery-section.ts`, `use-journey-section.ts`).

- [x] **Phase 4: Refactor Patient (User) Portal Components**
  - [x] Modularized views exceeding 150 lines (`appointment-detail-view.tsx`, `patient-appointments-view.tsx`).
  - [x] Modularized components exceeding 150 lines (`appointment-schedule-card.tsx`, `authenticated-user-header.tsx`).
  - [x] Extracted status history, appointment tab, status callout, and authenticated user menu sub-components while keeping view files under the 150-line threshold.

- [x] **Phase 5: Re-architect Secretary Portal Pages (Server Components + Views)**
  - [x] Convert all secretary sub-directory pages from Client Components to Server Components.
  - [x] Create respective client view entry files for `SecretaryAppointmentsView`, `SecretaryBookAppointmentView`, `SecretaryCheckInOutTrackerView`, `SecretaryInquiriesQueueView`, `SecretaryPendingRequestsView`, `SecretaryRescheduleRequestsView`, `SecretaryAuditLogView`, `SecretaryEmailLogView`, `SecretaryInvoiceManagementView`, and `SecretaryProfileView`.
  - [x] Ensure all view orchestrators and components are strictly presentational and hook-bound.
  - [x] Move state management and logical workflows from pages to dedicated custom hooks under `src/modules/staff/hooks/`.
  - [x] Break down the giant tables, forms, and modals into separate components under 150 lines.
  - [x] Double-verify all action calls: Backend Server Actions and DTO interfaces were not modified; frontend refactors preserved existing call payload shapes.

## Implementation Notes

- Created branch `codex/task-audit-works6`.
- Backend server action internals and DTO contracts were not modified.
- Standardized custom hook filenames; `rg "\.hook" samson-nextjs/src` returns no matches after the rename pass.
- Secretary route `page.tsx` files are now Server Component wrappers; client implementations were moved to module views under `src/modules/staff/views/secretary/`.
- Small secretary views completed with companion hooks and sibling specs: audit log, email log, invoice management, and profile.
- Reschedule requests completed with a companion hook, sibling hook spec, and focused list/detail/schedule/decision sub-components under 150 lines.
- Pending requests completed with a companion hook, sibling hook spec, and focused list/overview/schedule/edit/decision sub-components under 150 lines.
- Book appointment completed with a companion hook, sibling hook spec, and focused patient identity/schedule/success/toast sub-components under 150 lines. The book hook now uses the shared booking scheduler for date, doctor, and slot action delegation.
- Appointments directory completed with a companion hook, sibling hook spec, and focused tabs/filter/table/detail/reschedule/cancel/history sub-components under 150 lines. Availability loading now delegates through the shared booking scheduler while preserving the existing status update payloads.
- Inquiries queue completed with a companion hook, sibling hook spec, and focused list/profile/decision/context/schedule/linking/notes/toast sub-components under 150 lines. Availability loading now delegates through the shared booking scheduler while preserving convert/drop action payloads.
- Check-in/out tracker completed with a companion hook, sibling hook spec, and focused header/board/loading/checkout/reschedule/details sub-components under 150 lines. Realtime subscription, check-in, undo, no-show, checkout, and reschedule payloads were moved unchanged into the hook.
- Patient portal and landing component files identified in this audit are now split below the 150-line component threshold.
- Landing contact inquiry controller now uses Zod + React Hook Form while preserving the existing contact section prop contract and submit action payload.
- Verification attempted:
  - `rg "\.hook" samson-nextjs/src` returns no matches.
  - `rg "'use client'" "samson-nextjs/src/app/(portals)/secretary" -g "page.tsx"` returns no matches.
  - Bundled `pnpm install --frozen-lockfile` repaired dependencies from the lockfile after the local install was partially recreated.
  - `node .\node_modules\typescript\bin\tsc --noEmit --pretty false` passes when run outside the sandbox; the sandbox cannot open pnpm junction package files and reports `EPERM`.
  - Full `pnpm test` was intentionally skipped after the objective update requested not to run it for now. Newly added `renderHook` specs were aligned with existing test conventions by adding `@vitest-environment jsdom` headers.
