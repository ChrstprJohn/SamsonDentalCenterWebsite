...
const { mockConvertInquiry, mockGetServiceDuration } = vi.hoisted(() => {
  return {
    mockConvertInquiry: vi.fn(),
...
vi.mock('../../use-cases/booking/convert-inquiry.use-case', () => {
  return {
    convertInquiryUseCase: () => mockConvertInquiry,
  };
});

describe('convertInquiryAction', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('should validate inputs, check user role, run use case and return success', async () => {
    ...
    mockConvertInquiry.mockResolvedValueOnce({ appointmentId: 'app-uuid' });
