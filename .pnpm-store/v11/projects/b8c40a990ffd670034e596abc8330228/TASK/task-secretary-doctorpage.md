...
- [x] **Database & Schema Configuration**:
  - [x] Run a migration or execute DDL to add a `status` column to the `users` table with type `TEXT` and constraint `CHECK (status IN ('ACTIVE', 'HIDDEN', 'ARCHIVED'))` defaulting to `'ACTIVE'`.
    - Migration: `migrations/20260628070000_add_status_to_users.sql` ✅ (apply in Supabase dashboard)
  - [x] Ensure any seed files or existing doctor profiles are populated with `'ACTIVE'` status.
    - Pre-migration fallback: `status IS NULL` is treated as `'ACTIVE'` in all queries ✅
- [x] **Server Actions & Schemas (Backend)**:
  - [x] Update `use-doctor-form-schema.ts` to replace the `isActive` boolean with `status: z.enum(['ACTIVE', 'HIDDEN', 'ARCHIVED'])`. ✅
  - [x] Update `create-doctor.action.ts` to write the selected status (default `'ACTIVE'`) to both the database `users.status` column and Auth `user_metadata.status`. ✅
  - [x] Update `update-doctor.action.ts` to handle updating the `status` column in `users` and `user_metadata`. ✅
- [x] **Frontend Integration (Secretary Portal)**:
  - [x] Update `doctor-edit-form.tsx` to replace the operational status checkbox with a dropdown selector or radio option group for `ACTIVE`, `HIDDEN`, and `ARCHIVED`. ✅
  - [x] Update `doctor-card.tsx` to show status badges: `ACTIVE` (Green), `HIDDEN` (Amber/Orange - "Hidden on Website"), and `ARCHIVED` (Red - "Retired/Inactive"). ✅
  - [x] Update quick status selector in `doctor-details-pane.tsx` to support the three statuses. ✅
  - [x] Update roster search and status filter options in `use-doctor-management.ts` and `doctor-list.tsx` to handle the new states. ✅
- [x] **Booking Availability Rules**:
  - [x] **Online Booking (Public)**: Filter out doctors with status `HIDDEN` or `ARCHIVED` from the public availability and booking paths.
    - Fixed root cause: `isStaff` session detection was leaking — replaced with explicit `includeHidden` flag ✅
    - `get-doctor-schedules.queries.ts`: in-code filter, default ACTIVE only ✅
    - `get-working-schedules-for-month.queries.ts`: in-code filter, default ACTIVE only ✅
    - `get-active-doctors.queries.ts`: PostgREST OR filter, default ACTIVE only ✅
  - [x] **Manual Booking (Secretary)**: Filter out `ARCHIVED` doctors from the booking selectors, but keep `HIDDEN` doctors select-able for internal bookings.
    - `get-available-doctors-for-date.action.ts`: passes `includeHidden: true` ✅
    - `use-secretary-appointments.ts`, `use-secretary-pending-requests.ts`, `use-secretary-reschedule-requests.ts`: pass `includeHidden: true` ✅
  - [x] Update corresponding queries and actions returning doctor rosters (e.g. `getAvailableDoctors`, `getAvailableDays`, or related RPCs). ✅
- [x] **Verification**:
  - [x] Write/update unit tests in `use-doctor-form.spec.ts` for status Zod validations. ✅ (755 tests passing)
