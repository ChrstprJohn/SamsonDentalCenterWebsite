...
- [x] Refactor the User Portal Sidebar (`src/app/(portals)/user/layout.tsx`) to feature:
  - 📊 **My Dashboard** (`/user`)
...
- [x] Refactor `/user/page.tsx` and create a dedicated Dashboard View:
...
- [x] Create `/user/appointments/page.tsx`.
- [x] Refactor the existing dashboard view into a tabbed `PatientAppointmentsView` containing:
...
- [x] Create `/user/notifications/page.tsx` and `NotificationsView` component.
- [x] Show notification items with custom status badges/icons (appointment, reminder, announcement).
