...
export default async function DoctorPortalLayout({
  ...
  try {
    ...
    isAuthorized = role === 'DOCTOR' || role === 'ADMIN';

    headerUser = {
      firstName: user.user_metadata?.first_name || user.user_metadata?.firstName || 'Doctor',
    ...
    console.error('Doctor portal auth check failed, redirecting:', err);
    redirect('/auth/login');
  }
  ...
  try {
    ...
    console.error('Failed to load clinic config in doctor portal:', err);
  }
        ...
        <main className="flex-1 flex flex-col items-center justify-center p-6 text-center gap-6 pt-[120px]">
          ...
          <div className="flex flex-col gap-2 max-w-md">
            ...
            <p className="text-xs text-text-muted leading-relaxed">
              Your patient account lacks necessary clinical doctor credentials to access the Doctor operatory dashboard. Please contact a roster administrator if you require clinical clearance.
            </p>
          </div>
          <Link href="/user">
