...
export function ServiceCard({ nr, title, image, index, onClick }: ServiceCardProps) {
  ...
  return (
    <div onClick={onClick} title="Click to book this service" className={`group relative ${height} rounded-none overflow-hidden block ${colSpan} bg-[#1D1E1E] cursor-pointer`}>
      ...
      <span className="absolute top-3 left-3 sm:top-5 sm:left-5 md:top-4 md:left-4 lg:top-6 lg:left-6 text-white font-josefin font-normal text-base sm:text-lg md:text-base lg:text-2xl z-15">
        {nr}
      </span>
      ...
      <div className="absolute bottom-3 left-3 sm:bottom-5 sm:left-5 md:bottom-4 md:left-4 lg:bottom-6 lg:left-6 z-15 max-w-[85%]">
        <h3 className="font-josefin text-left text-lg sm:text-xl md:text-xl lg:text-3xl font-normal text-white tracking-tight leading-tight">
