...
export const onRescheduleBookingSmsSubscriber = {
  /**
   * Handles RESCHEDULE_BOOKING_SMS outbox events.
   * Dispatches Reschedule SMS notification if confirmation channel allows SMS.
   */
  async handle(payload: Record<string, any>): Promise<void> {
    ...
    if (appError || !appointment) {
      console.warn(`[Reschedule SMS] Appointment ${appointmentId} not found.`);
      return;
    }

    const channel = (appointment as any).confirmation_channel || 'EMAIL';
    if (channel === 'NONE' || channel === 'EMAIL') {
      console.info(`[Reschedule SMS] Skipping SMS for appointment ${appointmentId}: Channel is ${channel}.`);
      return;
    }
    ...
    if (!phone) {
      console.info(`[Reschedule SMS] Skipping SMS for appointment ${appointmentId}: No phone number registered.`);
      return;
    }
