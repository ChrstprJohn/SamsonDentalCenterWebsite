...
export const onNoShowSmsSubscriber = {
  /**
   * Handles APPOINTMENT_NO_SHOW_SMS outbox event.
   * Logs/dispatches Missed Appointment (No-show) SMS to patient if confirmation channel allows SMS.
   */
  async handle(payload: Record<string, any>): Promise<void> {
    ...
    if (appError || !appointment) {
      console.warn(`[No-Show SMS] Appointment ${appointmentId} not found.`);
      return;
    }

    const channel = (appointment as any).confirmation_channel || 'EMAIL';
    if (channel === 'NONE' || channel === 'EMAIL') {
      console.info(`[No-Show SMS] Skipping SMS for appointment ${appointmentId}: Channel is ${channel}.`);
      return;
    }
    ...
    if (!phone) {
      console.info(`[No-Show SMS] Skipping SMS for appointment ${appointmentId}: No phone number registered.`);
      return;
    }
