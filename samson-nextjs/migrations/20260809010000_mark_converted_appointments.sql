-- Migration: Mark converted appointments with CONVERTED source + update convert RPC
-- Date: 2026-08-09
-- Backfill existing converted appointments (linked via appointment_inquiries.linked_appointment_id),
-- then redefine convert_inquiry_to_appointment to stamp CONVERTED instead of STAFF_CREATED.

-- 1. Backfill: converted inquiries became appointments with source = STAFF_CREATED
UPDATE public.appointments a
SET source = 'CONVERTED'::public.appointment_source
FROM public.appointment_inquiries ai
WHERE ai.linked_appointment_id = a.id
  AND a.source = 'STAFF_CREATED'::public.appointment_source;

-- 2. Redefine convert RPC so future conversions stamp CONVERTED
