...
export function DoctorCard({ doctor, isSelected, onClick }: DoctorCardProps) {
  const getStatusDetails = (status: Doctor['status'], isActive: boolean) => {
    if (status === 'ARCHIVED') {
      return { label: 'ARCHIVED', className: 'bg-red-500/10 text-red-500 border-red-500/20' };
    }
    if (status === 'HIDDEN') {
      return { label: 'HIDDEN', className: 'bg-amber-500/10 text-amber-500 border-amber-500/20' };
    }
    if (status === 'FORCE_PASSWORD_CHANGE') {
      return { label: 'RESET REQ', className: 'bg-orange-500/10 text-orange-500 border-orange-500/20' };
    }
