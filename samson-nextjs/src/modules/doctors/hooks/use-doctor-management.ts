...
export interface Doctor {
  ...
  status: 'ACTIVE' | 'FORCE_PASSWORD_CHANGE' | 'INACTIVE';
  isActive: boolean;
  services: string[];
...
export function useDoctorManagement({ initialDoctors }: UseDoctorManagementProps) {
  ...
  const filteredDoctors = useMemo(() => {
    return initialDoctors.filter((doc) => {
      ...
      const matchesStatus =
        statusFilter === 'ALL' ||
        (statusFilter === 'ACTIVE' && doc.isActive && doc.status !== 'FORCE_PASSWORD_CHANGE') ||
        (statusFilter === 'INACTIVE' && !doc.isActive) ||
        (statusFilter === 'FORCE_PASSWORD_CHANGE' && doc.status === 'FORCE_PASSWORD_CHANGE');
