...
interface BadgeProps {
  ...
  variant?: 'success' | 'warning' | 'info' | 'cyan' | 'violet' | 'error' | 'default';
  className?: string;
}
...
export function Badge({ children, variant = 'default', className = '' }: BadgeProps) {
  ...
  const variants = {
    ...
    violet: 'bg-violet-500/10 text-violet-600 border-violet-500/20 dark:text-violet-400 dark:border-violet-500/30',
